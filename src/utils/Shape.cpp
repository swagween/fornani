...
namespace shape {
    ...
    {
        return (Vec(shape.position.x + shape.dimensions.x / 2.f, shape.position.y + shape.dimensions.y / 2.f));
    }

    Shape::Vec Shape::getThisCenter()
    {
        return (Vec(position.x + dimensions.x / 2.f, position.y + dimensions.y / 2.f));
    }
    ...
    bool Shape::AABB_is_left_collision(const Shape& immovable) {
        if (immovable.vertices.size() < 4) {
            ...
            return (position.x - epsilon < immovable.vertices[1].x &&
                (position.y + dimensions.y) > vertices[1].y + small_value &&
                position.y < vertices[2].y - small_value &&
                position.x + dimensions.x > immovable.vertices[1].x);
        }
    }
    ...
    bool Shape::AABB_is_right_collision(const Shape& immovable) {
        if (vertices.size() < 4) {
            ...
            return (position.x + dimensions.x + epsilon > immovable.vertices[0].x &&
                (position.y + dimensions.y) > vertices[1].y + small_value &&
                position.y < vertices[2].y - small_value &&
                position.x < immovable.vertices[0].x);
        }
    }
