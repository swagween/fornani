...
public:
    ...
    void render(sf::RenderWindow& win) {
        ...
        for(auto& digit : digits) {
            ...
            svc::assetLocator.get().sp_alphabet.at(lookup::get_character.at(digit)).setPosition(position.x + ORB_origin.x + PAD + (12 * ctr) + 2, position.y + ORB_origin.y + 2);
            win.draw(svc::assetLocator.get().sp_alphabet.at(lookup::get_character.at(digit)));
