...
void Caution::avoid_ledges(world::Map& map, shape::Collider& collider, int height) {
	...
	if (collider.flags.state.test(shape::State::ledge_right)) { retreat.x *= -1.f; }
}
bool entity::Caution::danger(dir::Direction& direction) const { return (retreat.x < -0.1f && direction.lr == dir::LR::right) || (retreat.x > 0.1f && direction.lr == dir::LR::left); }
} // namespace entity