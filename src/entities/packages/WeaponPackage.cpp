...
WeaponPackage::WeaponPackage(automa::ServiceProvider& svc, int id) { weapon = std::make_unique<arms::Weapon>(svc, id); }
...
void WeaponPackage::update(automa::ServiceProvider& svc, world::Map& map, enemy::Enemy& enemy) {
	...
	weapon.get()->firing_direction = enemy.get_direction();
}
