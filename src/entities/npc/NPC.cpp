...
NPC::NPC(automa::ServiceProvider& svc, int id) : id(id) {
	...
	idle.lookup = in_data["animation"]["idle"][0].as<int>();
	idle.duration = in_data["animation"]["idle"][1].as<int>();
	idle.framerate = in_data["animation"]["idle"][2].as<int>();
	idle.num_loops = in_data["animation"]["idle"][3].as<int>();
	idle.repeat_last_frame = (bool)in_data["animation"]["idle"][4].as_bool();
...
void NPC::set_id(int new_id) { id = id; }
