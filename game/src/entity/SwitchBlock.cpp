...
SwitchBlock::SwitchBlock(fornani::automa::ServiceProvider& svc, dj::Json const& in) : Entity(svc, in, "switch_blocks") {
	unserialize(in);
	p_animatable.set_texture_rect(sf::IntRect{{fornani::constants::i_cell_resolution * m_type, 0}, fornani::constants::i_resolution_vec});
	repeatable = true;
}

SwitchBlock::SwitchBlock(fornani::automa::ServiceProvider& svc, int id, int type) : Entity(svc, "switch_blocks", id), m_type{type} {
	p_animatable.set_texture_rect(sf::IntRect{{fornani::constants::i_cell_resolution * type, 0}, fornani::constants::i_resolution_vec});
	repeatable = true;
}
