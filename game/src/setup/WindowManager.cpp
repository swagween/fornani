...
void WindowManager::create(std::string const& title, bool const fullscreen) {
	...
	m_window = std::make_unique<sf::RenderWindow>(mode, title, is_fullscreen ? sf::Style::Default : sf::Style::Default & ~sf::Style::Resize, is_fullscreen ? sf::State::Fullscreen : sf::State::Windowed);
