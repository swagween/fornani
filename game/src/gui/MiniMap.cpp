...
MiniMap::MiniMap(automa::ServiceProvider& svc) : m_texture(svc), m_map_sprite{svc.assets.t_null}, m_cursor{svc.assets.get_texture("map_cursor")} {
	...
	m_border.setSize(svc.constants.f_screen_dimensions);
...
void MiniMap::bake(automa::ServiceProvider& svc, world::Map& map, player::Player& player, int room, bool current, bool undiscovered) {
	...
	if (current) {
		m_player_position = player.collider.get_center() * m_texture_scale / svc.constants.cell_size + room_pos;
...
void MiniMap::render(automa::ServiceProvider& svc, sf::RenderWindow& win, player::Player& player, sf::Vector2f cam, sf::Sprite& icon_sprite) {
	...
	for (auto& room : m_atlas) {
		...
		m_map_sprite.setPosition((room->get_position() * get_ratio() + m_position).componentWiseDiv(port.size));
		...
		for (auto i{-1}; i < 2; ++i) {
			for (auto j{-1}; j < 2; ++j) {
				...
				outline.setPosition((room->get_position() * get_ratio() + m_position).componentWiseDiv(port.size) + (skew * 2.f).componentWiseDiv(port.size));
				win.draw(outline);
			}
		}
	...
	for (auto& element : m_markers) {
		...
		icon_sprite.setPosition((element.position * get_ratio() + m_position).componentWiseDiv(port.size));
