...
HUDAmmo::HUDAmmo(automa::ServiceProvider& svc, player::Player& player, std::string_view tag, sf::Vector2f root, sf::Vector2i dimensions)
	: HUDWidget{svc, player, tag, root, dimensions}, m_reload_bar{svc, {64.f, 8.f}, {colors::blue, colors::goldenrod, colors::navy_blue}, false} {}
...
void HUDAmmo::update(automa::ServiceProvider& svc, player::Player& player) {
	...
	if (player.arsenal && player.hotbar) {
		if (player.switched_weapon() || Flaggable<HUDAmmoFlags>::consume_flag(HUDAmmoFlags::switched)) {
			m_ammo_bar = WidgetBar(svc, player.equipped_weapon().ammo.get_capacity(), p_animatable->get_dimensions(), svc.assets.get_texture("hud_ammo"), get_root(), 2.f, true);
		}
		player.hotbar->sync();
		...
		if (m_ammo_bar) {
			...
			m_reload_bar.update(svc, get_root() + sf::Vector2f{0.f, 32.f}, player.equipped_weapon().get_reload().get_inverse_normalized());
		}
	}
}
...
auto HUDAmmo::get_offset() const -> sf::Vector2f { return HUDWidget::get_offset() + sf::Vector2f{0.f, m_reload_bar.get_dimensions().y}; }
