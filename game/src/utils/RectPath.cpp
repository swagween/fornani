...
RectPath::RectPath(data::ResourceFinder& finder, std::filesystem::path source, std::string_view type) : m_scale{2.f}, interpolation{8} {
...
void RectPath::update() {
	...
	m_current_position.x = std::lerp(previous_position.x, target_position.x, interpolation.get_inverse_normalized());
	m_current_position.y = std::lerp(previous_position.y, target_position.y, interpolation.get_inverse_normalized());
	m_current_dimensions.x = std::lerp(previous_dimensions.x, target_dimensions.x, interpolation.get_inverse_normalized());
	m_current_dimensions.y = std::lerp(previous_dimensions.y, target_dimensions.y, interpolation.get_inverse_normalized());
