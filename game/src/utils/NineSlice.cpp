...
void NineSlice::render(sf::RenderWindow& win, sf::Vector2f cam) {
	...
	m_sprite.setPosition(m_steering.physics.position - cam + m_render_offset);
	win.draw(m_sprite);
	// top right
	...
	m_sprite.setPosition(m_steering.physics.position + sf::Vector2f{f_dim.x, 0.f} - cam + m_render_offset);
	win.draw(m_sprite);
	// bottom left
	...
	m_sprite.setPosition(m_steering.physics.position + sf::Vector2f{0.f, f_dim.y} - cam + m_render_offset);
	win.draw(m_sprite);
	// bottom right
	m_sprite.setOrigin({});
	m_sprite.setTextureRect(sf::IntRect{{m_corner_dimensions + m_edge_dimensions}, m_corner_dimensions});
	m_sprite.setPosition(m_steering.physics.position + f_dim - cam + m_render_offset);
	win.draw(m_sprite);
