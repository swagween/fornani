...
void CircleCollider::handle_map_collision(world::Map& map) {
	...
	for (int i = 0; i < 4; ++i) { map.handle_cell_collision(*this); }
}
...
void CircleCollider::resolve_collision() {
	physics.position += m_correction;
	sensor.set_position(physics.position);

	if (m_collision_normal.lengthSquared() > constants::tiny_value) {
		auto const normal = m_collision_normal.normalized();
...
void CircleCollider::handle_collision(shape::Shape const& shape, bool soft) {
	...
	if (shape.non_square()) {
		auto const cmtv = shape.circle_SAT_MTV(sensor.bounds);
		physics.position += cmtv;
		auto const normal = cmtv.normalized();
		auto const velocity_normal = physics.velocity.dot(normal);
		if (velocity_normal < 0.f) { physics.velocity -= (1.f + physics.elasticity) * velocity_normal * normal; }
		if (cmtv.lengthSquared() > constants::small_value) { m_flags.set(CircleColliderFlags::collided); }
	} else {
