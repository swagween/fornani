...
PlayerAnimation::PlayerAnimation(Player& plr) : m_player(&plr), state_function{std::bind(&PlayerAnimation::update_idle, this)}, m_buffer{16}, m_sleep_timer{512}, m_ko_timer{80} {
	plr.p_animatable.set_animations({{"idle", {20, 8, 7 * nani_animation_rate_v, -1, false, true}},
									 ...
									 {"between_push", {84, 2, 6 * nani_animation_rate_v, 0}},
									 {"push", {86, 4, 6 * nani_animation_rate_v, -1}},
									 {"rise", {40, 4, 6 * nani_animation_rate_v, 0}},
									 {"walljump", {90, 6, 6 * nani_animation_rate_v, 0}},
									 {"corner_flip", {175, 6, 5 * nani_animation_rate_v, 0}},
									 ...
									 {"knock_over", {76, 3, 8 * nani_animation_rate_v, 0}},
	...
	m_player->p_animatable.animation.set_params(get_params("idle"));
	m_player->p_animatable.animation.start();
	request(AnimState::idle);
}

int PlayerAnimation::get_frame() const { return m_player->p_animatable.animation.get_frame(); }
...
bool PlayerAnimation::stepped() const {
	return (m_player->p_animatable.animation.get_frame() == 44 || m_player->p_animatable.animation.get_frame() == 46 || m_player->p_animatable.animation.get_frame() == 10 || m_player->p_animatable.animation.get_frame() == 13 ||
			m_player->p_animatable.animation.get_frame() == 16) &&
		   m_player->p_animatable.animation.keyframe_started();
}
...
fsm::StateFunction PlayerAnimation::update_idle() {
	m_player->p_animatable.animation.label = "idle";
	...
	if (change_state(AnimState::knock_over, get_params("knock_over"), true)) { return PA_BIND(update_knock_over); }
	if (m_player->p_animatable.animation.just_started()) {
		idle_timer.start();
	...
	if (change_state(AnimState::between_push, get_params("between_push"))) { return PA_BIND(update_between_push); }
...
fsm::StateFunction PlayerAnimation::update_sprint() {
	m_player->p_animatable.animation.label = "sprint";
...
fsm::StateFunction PlayerAnimation::update_shield() {
	m_player->p_animatable.animation.label = "shield";
...
fsm::StateFunction PlayerAnimation::update_between_push() {
	m_player->p_animatable.animation.label = "between push";
...
fsm::StateFunction PlayerAnimation::update_push() {
	m_player->p_animatable.animation.label = "push";
	...
	if (!was_requested(AnimState::push)) {
		m_player->cooldowns.post_push.start();
		request(AnimState::idle);
		if (change_state(AnimState::idle, get_params("idle"))) { return PA_BIND(update_idle); }
	}
...
fsm::StateFunction PlayerAnimation::update_run() {
	m_player->p_animatable.animation.label = "run";
...
fsm::StateFunction PlayerAnimation::update_turn() {
	m_player->p_animatable.animation.label = "turn";
...
fsm::StateFunction PlayerAnimation::update_sharp_turn() {
	m_player->p_animatable.animation.label = "sharp_turn";
...
fsm::StateFunction PlayerAnimation::update_rise() {
	m_player->p_animatable.animation.label = "rise";
	...
	if (m_player->p_animatable.animation.complete()) {
		...
		m_player->p_animatable.animation.set_params(get_params("suspend"));
...
fsm::StateFunction PlayerAnimation::update_suspend() {
	m_player->p_animatable.animation.label = "suspend";
	...
	if (change_state(AnimState::backflip, get_params("backflip"))) { return PA_BIND(update_backflip); }
	if (change_state(AnimState::wallslide, get_params("wallslide"))) { return PA_BIND(update_wallslide); }
	if (change_state(AnimState::corner_flip, get_params("corner_flip"))) { return PA_BIND(update_corner_flip); }
	if (change_state(AnimState::fall, get_params("fall"))) { return PA_BIND(update_fall); }
	if (change_state(AnimState::crouch, get_params("crouch"))) { return PA_BIND(update_crouch); }
	if (change_state(AnimState::crawl, get_params("crouch"), true)) { return PA_BIND(update_crouch); }
	if (change_state(AnimState::push, get_params("push"))) { return PA_BIND(update_push); }
...
fsm::StateFunction PlayerAnimation::update_fall() {
	m_player->p_animatable.animation.label = "fall";
	...
	if (m_player->has_collider()) {
		if (m_player->grounded()) {
			m_player->p_animatable.animation.set_params(get_params("land"));
...
fsm::StateFunction PlayerAnimation::update_stop() {
	m_player->p_animatable.animation.label = "stop";
...
fsm::StateFunction PlayerAnimation::update_inspect() {
	m_player->p_animatable.animation.label = "inspect";
...
fsm::StateFunction PlayerAnimation::update_sit() {
	m_player->p_animatable.animation.label = "sit";
...
fsm::StateFunction PlayerAnimation::update_land() {
	m_player->p_animatable.animation.label = "land";
...
fsm::StateFunction PlayerAnimation::update_hurt() {
	m_player->p_animatable.animation.label = "hurt";
...
fsm::StateFunction PlayerAnimation::update_dash() {
	m_player->p_animatable.animation.label = "dash";
...
fsm::StateFunction PlayerAnimation::update_dash_up() {
	m_player->p_animatable.animation.label = "dash up";
...
fsm::StateFunction PlayerAnimation::update_dash_down() {
	m_player->p_animatable.animation.label = "dash down";
...
fsm::StateFunction PlayerAnimation::update_wallslide() {
	m_player->p_animatable.animation.label = "wallslide";
...
fsm::StateFunction PlayerAnimation::update_walljump() {
	m_player->p_animatable.animation.label = "walljump";
...
fsm::StateFunction PlayerAnimation::update_die() {
	m_player->p_animatable.animation.label = "die";
	...
	if (m_player->has_collider()) {
		m_player->p_animatable.animation.linger_on_frame(1, !m_player->get_collider().grounded());
...
fsm::StateFunction PlayerAnimation::update_drown() {
	m_player->p_animatable.animation.label = "drown";
...
fsm::StateFunction PlayerAnimation::update_backflip() {
	m_player->p_animatable.animation.label = "backflip";
...
fsm::StateFunction PlayerAnimation::update_slide() {
	m_player->p_animatable.animation.label = "slide";
...
fsm::StateFunction PlayerAnimation::update_get_up() {
	m_player->p_animatable.animation.label = "get_up";
...
fsm::StateFunction PlayerAnimation::update_roll() {
	m_player->p_animatable.animation.label = "roll";
	...
	if (m_player->p_animatable.animation.complete()) {
		if (controller.is_sliding() && !m_player->firing_weapon()) {
			m_player->p_animatable.animation.set_params(get_params("slide"));
			return PA_BIND(update_slide);
		} else {
			if (controller.sprinting()) {
				m_player->p_animatable.animation.set_params(get_params("sprint"));
...
fsm::StateFunction PlayerAnimation::update_turn_slide() {
	m_player->p_animatable.animation.label = "turn_slide";
...
fsm::StateFunction PlayerAnimation::update_shoot() {
	m_player->p_animatable.animation.label = "shoot";
...
fsm::StateFunction player::PlayerAnimation::update_sleep() {
	m_player->p_animatable.animation.label = "sleep";
...
fsm::StateFunction PlayerAnimation::update_unconscious() {
	m_player->p_animatable.animation.label = "unconscious";
	p_state.actual = AnimState::unconscious;
	m_player->set_flag(PlayerFlags::show_weapon, false);
	m_player->controller.restrict_movement();
	m_player->controller.prevent_movement();
	if (m_player->has_collider() && !m_ko_timer.running()) { m_player->get_collider().physics.zero_x(); }
	if (m_sleep_timer.is_almost_complete() || m_ko_timer.is_almost_complete()) { request(AnimState::recover); }
...
fsm::StateFunction player::PlayerAnimation::update_wake_up() {
	m_player->p_animatable.animation.label = "wake_up";
...
fsm::StateFunction PlayerAnimation::update_recover() {
	m_player->p_animatable.animation.label = "recover";
...
fsm::StateFunction player::PlayerAnimation::update_crouch() {
	m_player->p_animatable.animation.label = "crouch";
...
fsm::StateFunction player::PlayerAnimation::update_crawl() {
	m_player->p_animatable.animation.label = "crawl";
	...
	if (change_state(AnimState::crouch, get_params("crouch"), true)) {
		m_player->p_animatable.animation.set_frame(4);
		return PA_BIND(update_crouch);
	}
	if (!m_player->controller.is_crouching()) {
		request(AnimState::get_up);
...
fsm::StateFunction player::PlayerAnimation::update_dash_kick() {
	m_player->p_animatable.animation.label = "dash_kick";
...
fsm::StateFunction player::PlayerAnimation::update_slow_walk() {
	m_player->p_animatable.animation.label = "slow_walk";
...
fsm::StateFunction player::PlayerAnimation::update_swim() {
	m_player->p_animatable.animation.label = "swim";
...
fsm::StateFunction player::PlayerAnimation::update_dive() {
	m_player->p_animatable.animation.label = "dive";
...
fsm::StateFunction PlayerAnimation::update_hover() {
	m_player->p_animatable.animation.label = "hover";
...
fsm::StateFunction PlayerAnimation::update_stun() {
	m_player->p_animatable.animation.label = "stun";
...
fsm::StateFunction PlayerAnimation::update_melee_front_kick() {
	m_player->p_animatable.animation.label = "melee_front_kick";
	p_state.actual = AnimState::melee_front_kick;
	if (m_player->has_collider() && m_player->p_animatable.animation.get_frame_count() == 1) { m_player->get_collider().physics.zero_x(); }
	if (change_state(AnimState::die, get_params("die"), true)) { return PA_BIND(update_die); }
	if (change_state(AnimState::drown, get_params("drown"), true)) { return PA_BIND(update_drown); }
	if (change_state(AnimState::stun, get_params("stun"), true)) { return PA_BIND(update_stun); }
	if (change_state(AnimState::knock_over, get_params("knock_over"), true)) { return PA_BIND(update_knock_over); }
	...
	if (change_state(AnimState::dash_up, get_params("dash_up"))) { return PA_BIND(update_dash_up); }
	if (change_state(AnimState::dash_down, get_params("dash_down"))) { return PA_BIND(update_dash_down); }
	if (m_player->p_animatable.animation.get_frame_count() > 1) {
...
fsm::StateFunction PlayerAnimation::update_melee_side_kick() {
	m_player->p_animatable.animation.label = "melee_side_kick";
...
fsm::StateFunction PlayerAnimation::update_melee_roundhouse_kick() {
	m_player->p_animatable.animation.label = "melee_roundhouse_kick";
...
fsm::StateFunction PlayerAnimation::update_drink() {
	m_player->p_animatable.animation.label = "drink";
...
fsm::StateFunction PlayerAnimation::update_knock_over() {
	m_player->p_animatable.animation.label = "knock_over";
	p_state.actual = AnimState::knock_over;
	if (m_player->has_collider()) {
		m_player->p_animatable.animation.linger_on_frame(1, !m_player->get_collider().grounded());
		m_player->get_collider().collision_depths = {};
	}
	...
	m_player->controller.prevent_movement();
	if (m_player->p_animatable.animation.complete()) {
		m_player->set_flag(PlayerFlags::knocked_over, false);
		m_ko_timer.start();
		request(AnimState::unconscious);
		if (change_state(AnimState::unconscious, get_params("unconscious"))) { return PA_BIND(update_unconscious); }
	}
	return PA_BIND(update_knock_over);
}

fsm::StateFunction PlayerAnimation::update_corner_flip() {
	m_player->p_animatable.animation.label = "corner_flip";
	p_state.actual = AnimState::corner_flip;
	if (change_state(AnimState::die, get_params("die"), true)) { return PA_BIND(update_die); }
	if (change_state(AnimState::drown, get_params("drown"), true)) { return PA_BIND(update_drown); }
	if (change_state(AnimState::stun, get_params("stun"), true)) { return PA_BIND(update_stun); }
	if (change_state(AnimState::knock_over, get_params("knock_over"), true)) { return PA_BIND(update_knock_over); }
...
bool PlayerAnimation::change_state(AnimState next, anim::Parameters params, bool hard) {
	if (was_requested(next)) {
		m_player->p_animatable.animation.set_params(params, true);
		return true;
	}
	return false;
}
