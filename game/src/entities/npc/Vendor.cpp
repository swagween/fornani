...
void Vendor::generate_inventory(automa::ServiceProvider& svc) {
	...
	for (auto const& gfi : guaranteed_finite_items) { inventory.add_item(svc.data, gfi); }
	for (auto i{0}; i < m_stock_size; ++i) {
		...
		if (rand < 6) {
			...
			inventory.add_item(svc.data, rare_items.at(choice));
		} else if (rand < 100) {
			auto const choice = random::random_range(0, static_cast<int>(uncommon_items.size() - 1), selection);
			inventory.add_item(svc.data, uncommon_items.at(choice));
		} else {
			...
			inventory.add_item(svc.data, common_items.at(choice));
		}
	}
}
