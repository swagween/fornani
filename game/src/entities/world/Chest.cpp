	...
	if (collider.collides_with(player.collider.bounding_box)) {
		if (player.controller.inspecting()) {
			...
			if (!state.test(ChestState::open)) {
				...
				if (m_type == ChestType::orbs) { map.active_loot.push_back(item::Loot(svc, player, {range_modifier, range_modifier * 2}, fmodifier, collider.get_global_center(), 100, true, map.get_special_drop_id())); }
