...
Animator::Animator(automa::ServiceProvider& svc, std::string label, sf::Vector2i pos, bool foreground)
	: m_sprite{svc.assets.get_texture(label + "_animators")}, m_position{sf::Vector2f{static_cast<float>(pos.x), static_cast<float>(pos.y)} * constants::f_cell_size}, m_foreground{foreground} {}
