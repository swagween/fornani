...
Chain::Chain(automa::ServiceProvider& svc, SpringParameters params, sf::Vector2f position, int num_links, bool reversed, float spacing, bool linked)
	: Chain(svc, svc.assets.get_texture("null"), params, position, num_links, reversed, spacing, linked) {}

vfx::Chain::Chain(automa::ServiceProvider& svc, sf::Texture const& tex, SpringParameters params, sf::Vector2f position, int num_links, bool reversed, float spacing, bool linked) : m_root(position), m_sprite{tex}, m_external_dampen{0.07f} {
	if (!linked) {
...
void Chain::update(automa::ServiceProvider& svc, world::Map& map, player::Player& player, float dampen) {
	...
	for (auto& link : links) {
		...
		if (mtv.length() > constants::small_value) { ++m_num_collisions; }
	...
	if (m_centroid) {
		...
		for (auto& link : links) { link.set_bob(link.get_bob() + offset * 0.2f); }
	}
}
...
void Chain::render(automa::ServiceProvider& svc, sf::RenderWindow& win, sf::Vector2f cam, bool average) {
	for (auto& link : links) {
		...
		m_sprite.setPosition(link.get_average_bob_position() - cam);
		win.draw(m_sprite);
	}
}
...
auto Chain::get_percentage_colliding() const -> float { return m_num_collisions / static_cast<int>(links.size()); }
