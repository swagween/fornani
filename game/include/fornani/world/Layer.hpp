...
class Layer {
  public:
	friend class Map;
	Layer(std::uint8_t o, sf::Vector2i partition, sf::Vector2<std::uint32_t> dim, dj::Json& source, float spacing, bool has_obscuring, bool has_reverse_obscuring, float parallax = 1.f, bool ignore_lighting = false, bool animated = false)
		...
		animated ? m_flags.set(LayerFlags::animated) : m_flags.reset(LayerFlags::animated);
	}
	...
	[[nodiscard]] auto is_animated() const -> bool { return m_flags.test(LayerFlags::animated); }
	bool collidable{};
  ...
  private:
	...
	std::optional<AnimatedLayerTexture> m_animated_texture{};
	std::optional<sf::RenderTexture> m_barrier{};
	util::Circuit m_current_frame{6};
	float m_parallax;
